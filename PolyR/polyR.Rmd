...
If it would be a list the values wouldn't update and would only return the values at the initialization of the parent function. Let's take our constructor function and see how that works practice.
...
makeRawSourceAlt <- function(ncolumns) {
  activeColumns  = 0:ncolumns
  position       = ncolumns
  prevPosition   = NA
  nactive        = ncolumns
...
setn = 5
testA <- makeRawSourceAlt(setn)
testB <- makeRawSource(setn)
...
while(setn >= 0){
   testA$generateFeature() 
   testB$generateFeature() 
   setn = setn-1
}
