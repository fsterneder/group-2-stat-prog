...
while (!reduce(map(expert, function(x) x$finished()), function(x,y) x || y) 
  ...
  debug <- paste("winner:", wstr , "index:", index, "wealth const:", expert$const$state()$wealth, "wealth geom: ", expert$geom$state()$wealth, "bid: ", maxBid, "p: ", pvals[min(unlist(index))])
}
...
What we essentially did was take every function, get rid of the wealth in the state and rely on a wealth value in the global environment. 
...
# we can simply pass the initalized function to our function
...
while (!reduce(map(expert, function(x) x$finished()), function(x,y) x || y) && wealth > 0) {
  ...
  debug <- paste("winner:",wstr ,"index:", index, "wealth:", wealth, "bid:", maxBid, "p:", pvals[min(unlist(index))])
}
