...
while (!reduce(expert, function(x,y) x$finished() || y$finished()) && reduce(expert, function(x,y) x$state()$wealth > 0 || y$state()$wealth > 0)) {
  bid = list(const = expert$const$bidder$bid(), geom = expert$geom$bidder$bid())
  ifelse(bid$const > bid$geom, winner <- expert$const, winner <- expert$geom)
  index = winner$feature()
  maxBid <- max(unlist(bid))
  winner$pay(maxBid)

  print(paste("index: ", index, "wealth const: ", expert$const$state()$wealth, "wealth geom: ", expert$geom$state()$wealth, "bid: ", maxBid))

  if (pvals[min(unlist(index))] < maxBid) {
