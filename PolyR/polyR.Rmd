...
#and 3 functions 
  list( 
    ...
    #these functions change the (initial) wealth of the parent environment
    #bidAccepted increases the wealth
    bidAccepted = function(a) { wealth <<- wealth + a },
    #bidRejected decreases the wealth
    bidRejected = function(d) { wealth <<- wealth - d },
    #at the end, the bidfunction is called, which either returns the wealth, or the wealth*frac
    bid = function()  { ifelse(wealth > .0001, wealth*frac, wealth) }
  )
}
...
#this function has again default values. It is the same as the one before, only the bid-function differs. 
  list( 
    ...
    #bidRejected decreases the wealth
    bidRejected = function(d) { wealth <<- wealth - d },
    #at the end, the bidfunction is called, which either returns the wealth, or the const
    bid = function()  { ifelse(wealth >= const, const, wealth) }  
  )
}
...
#every column is stored as an active column (WHY DOES IT START WITH 0?)
  activeColumns  = 0:ncolumns
  #a positions variable is made for ??
  #position will be changed in generateFeature-function
  position       = ncolumns
  #the previous position is NA at the beginning (will be changed with following generateFeature-function))
  prevPosition   = NA
  nactive        = ncolumns
