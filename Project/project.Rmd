...
Section 1 is concerened with handedness. More specifically, we ask whether the distribution of right-, left- and both-handed players has changed since the MLB's inception in 1871, and whether a player's expected value to his team differed as a function of handedness in a single season (2016).  
...
In Section 3, we looked at player salaries. How did player salaries change over years? Are there any differences between leagues with respect to player salaries? How much did the most paid players earn? How much did the most paying teams pay? And, how do baseball players salaries compare to the average US income?  

Finally, in section 4 we use handedness, age, and salary, and build a linear model to predict a player's expected value to his team given these covariates. 
...
ggplotly(bat_plot, tooltip = c("proportion", "yearID", "N")) 
...
ggplotly(throw_plot,tooltip = c("proportion", "yearID", "N"))
```    
...
We then joined ```appearances_2016``` and ```warPos2016``` using player and team IDs as keys. 
```{r}
#Combine appearances_2016 and warPos2016 tables
pos_2016 <- pos_2016 %>% 
  left_join(war_pos_2016, by = 
...
salaries16_pl_warPos <- inner_join(salaries16, warPos16, "playerID")
(anti_join(salaries16, warPos16, "playerID"))
(anti_join(warPos16, salaries16, "playerID"))

#using an inner join to only hold the rows with WAR and salary
salaries16_pl_warPit <- inner_join(salaries16, warPit16, c("playerID", "yearID", "teamID", "lgID"))
#using anti_joins in both directions to see the data loss
(anti_join(salaries16, warPit16, "playerID") %>% print())
(anti_join(warPit16, salaries16, "playerID") %>% print())
```
  
####Salary and Wins above replacement
Then we start the modeling. First we look at how WAR affects the salary. 
```{r lm_war_salary}
lm_warpos_salary <- lm_own(salaries16_pl_warPos, "salary", "WARpos", 
...
lm_warpit_salary <- lm_own(salaries16_pl_warPit, "salary", "WARpit", 
...
```{r lm_salary_war}
lm_salary_warpos <- lm_own(salaries16_pl_warPos, "WARpos", "salary",
...
lm_salary_warpit <- lm_own(salaries16_pl_warPit, "WARpit", "salary",
...
lm(data = salaries16_pl_warPit, formula = WARpit ~ salary)
stargazer(lm(data = salaries16_pl_warPit, formula = WARpit ~ salary),
          type = "HTML")
```
...
(anti_join((Teams %>% filter(yearID == age_year) %>% 
                      select(teamID,G, W, L)) %>% 
...
```{r}
#preparing the data
baseball16_bat <- inner_join(salaries16, age_c_players, by = c("playerID", "teamID")) %>%
  print() 
  #hier noch inner_join mit handedness (bat) hinzufügen

anti_join(salaries16, age_c_players, by = c("playerID", "teamID")) #drop 48 rows (41 without teamID)
anti_join(age_c_players, salaries16, by = c("playerID", "teamID")) #drop 677 rows (581 without teamID)

baseball16_throw <- inner_join(salaries16, age_c_players, by = c("playerID", "teamID")) %>%
  print() 
  #hier noch inner_join mit handedness (throw) hinzufügen
