...
$$
```{r}
modelBatsPos <- lm(weightedWAR~bats, data = pos_2016)
```

```{r results = "asis"}
stargazer(modelBatsPos, type = "html", align = T)
```
...
$$  
```{r}
modelThrowsPos <- lm(weightedWAR~throws, data = pos_2016)
```

```{r results = "asis"}
stargazer(modelThrowsPos, align = T, type = "html")
```
