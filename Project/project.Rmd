...
age_c_team %>% 
    ...
    guides(color = FALSE) + labs(x = "Team", y = "Average age") +
...
age_c_team %>% 
    ...
    guides(color = FALSE) + 
    labs(y = "Age Weighted By Games Played", x = "Team") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
    coord_flip()
```
...
age_plot <- age_c_team %>% 
  ...
  labs(x = "Age Weighted By Games Played", y = "% Games won")
div(ggplotly(age_plot), align = "center")
...
age_plot <- age_c_team %>% 
  ...
  geom_point() +
  labs(y = "Residuals", x = "Age Weighted By Games Played")
div(ggplotly(age_plot), align = "center")
```
...
## Age and Wins Above Replacement - Position

### Prep

```{r}
age_war <- left_join(age_c_players, pos_2016 %>%
...
`r dim(age_war %>% filter(is.na(weightedWAR)))[1]` out of `r dim(age_war)[1]`.

```{r}
age_war <- age_war %>% drop_na(weightedWAR)
```
...
```{r}
age_war_mod <- age_war %>% lm(formula = weightedWAR ~ age)
age_war <- age_war %>% add_predictions(age_war_mod) %>% add_residuals(age_war_mod)
...
```{r results="asis"}
stargazer(age_war_mod, align=T, type='html')
```

## Age and Wins Above Replacement - Pitcher

### Prep

```{r}
age_war <- left_join(age_c_players, pit_2016 %>%
  select(-bats,-throws,-data)) %>%
  group_by(playerID,birthDate,age,weightedWAR) %>% 
  summarise(G_all = sum(G_all), GS = sum(GS)) %>%
  ungroup()
...
age_war <- left_join(age_c_players, combo_2016 %>%
  select(-bats,-throws,-data)) %>%
  group_by(playerID,birthDate,age,weightedWAR) %>% 
  summarise(G_all = sum(G_all), GS = sum(GS)) %>%
  ungroup()
...
finalModel <- lm(weightedWAR~bats+age+salary, data=df16)
