...
Before starting the analysis, we have to prepare our data. For the salaries, we have the table `Salaries_US` containing the average wages from 1989-2017, and another table `Salaries` containing all baseball-players' salaries from 1989-2016. In both tables, we look at the (average) million US$ earned per year. 
...
sal_bb_nbplayers <- sal_bb_stats %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
  scale_x_continuous(breaks = Salaries$yearID) +
  labs(x = "Year", y = "Number of players", color = "Number of players",
       title = "Number of players with salary information each year") +
  scale_color_manual(labels = c("Distinct number \nof players", "Number of players"), values = c("#384F5d", "#3068A1"))
(sal_bb_nbplayers)
...
To find out about the relationship of salary and being a good player we'll construct a linear model. We will compare the players' salaries to the Wins above replacement for position players and for pitching. To shorten the code, we made a function to calculate our linear model (saved in help.R). 
