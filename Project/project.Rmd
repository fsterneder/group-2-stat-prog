...
##Linear Model
For our final linear models we considered all of the variables above as covariates to predict a player's weighted WAR. Also, we included an interaction term for age and salary. We suspected the strength of the relationship between salary and weighted WAR may vary as a function of age. More precisely, younger players who are under their first contract could be very important to their teams, however get paid relatively less than older players who are beyond their first contracts. That is, we consider it reasonable to suspect that older, established players (beyond their first contract) are paid according to their actual performance, whereas younger players are paid according to their potential. We also included handedness as a covariate, with a player's throwing hand as a proxy for "handedness".  
...
$$   
```{r results="asis"}
stargazer(finalModel, align = T , type="html")
```  
Unsurprisingly (given the previous linear models) our final model does not "explain" the data very well ($R^2$ < .06). 

We also plotted the residuals against all four covariates to see if there's any remaining trend in our data that is not accounted for by the model (presumably there is).
...
(hand_plot <- ggplot(df16, aes(pred, resid))+
  geom_point()+
  ggtitle("Predicted Values vs. Residuals")+
  xlab("Predicted")+
  ylab("Residuals"))
```
The scatter plot for predicted values and residuals looks nothing like random noise. It is most likely that there is a pattern in the data that was not captured by our model. 

##Visualizing the Final Model  
As a final gimmick, we visualized our model using a 3D plot. 
