...
age_current_players <- 
    ...
    mutate(age = age_calc(birthDate,ymd(age_def))) %>%
    select(playerID, teamID, birthDate, age, nameFirst, nameLast, G_all, GS) 
```

We find that the average active professional baseball player in 2018 was born `r mean(age_current_players$birthDate)` and was therefore `r age_calc(mean(age_current_players$birthDate),ymd(age_def))` years old. 
...
Funfact: As we might be able to tell from the graph, the oldest player in the season of 2018 was [`r age_calc(min(age_current_players$birthDate),ymd(age_def))`](https://en.wikipedia.org/wiki/Bartolo_Col%C3%B3n).
...
```{r}
names(color$team_main) <- sort(unique(age_current_players$teamID))
names(color$team_second) <- sort(unique(age_current_players$teamID))

age_current_team <- age_current_players %>% 
   group_by(teamID) %>%
   summarise(meanAge = mean(age), medianAge = median(age)) %>%
   left_join((Teams %>% filter(yearID == 2018) %>% select(teamID,name)),by="teamID")
...
```
So now we know that the the team with the - on average - youngest players are the `r (age_current_team %>% filter(meanAge == min(meanAge)))$name` while the `r (age_current_team %>% filter(meanAge == max(meanAge)))$name` deploys -again, on average- the oldest players. But the difference between these 'extremes' is `r signif(max(age_current_team$meanAge) - min(age_current_team$meanAge),3)` and in my view not that siginificant. 

We still want to know more, let's look at the related boxplot.

```{r}
age_current_players %>% 
    left_join(age_current_team, by="teamID") %>%
...
age_current_team <- 
   left_join(
             age_current_players,
             (Teams %>% filter(yearID == "2018") %>% select(teamID,G)),
             "teamID"
            ) %>%
   group_by(teamID) %>% 
   add_count() %>%
   ...
   summarise(gp = mean(gProp), gsp = mean(gsProp)) %>%
   left_join(age_current_team, "teamID")

age_current_team %>% 
    ggplot(aes(reorder(teamID,meanAge),gp,color=teamID)) +
